...
#####################################################################
# Save dfs into csv
ice_cost.to_csv('dataframe_ice_cost.csv', index=False)
EV40_cost.to_csv('dataframe_EV40_cost.csv', index=False)
EV60_cost.to_csv('dataframe_EV60_cost.csv', index=False)
transit_cost.to_csv('dataframe_transit_cost.csv', index=False)
...
df = pd.concat([ice_cost, EV40_cost, transit_cost])
print(df.columns)
...
weighted_GHG_cost_190 = repeat_values(df['GHG_cost_190'], df['Count'])
weighted_Air_pollutant_cost = repeat_values(df['Air_pollutant_cost'], df['Count'])
weighted_Collision_cost = repeat_values(df['Collision_cost'], df['Count'])
weighted_Congestion_cost = repeat_values(df['Congestion_cost'], df['Count'])
weighted_Noise_cost = repeat_values(df['Noise_cost'], df['Count'])
weighted_Externality = repeat_values(df['Externality'], df['Count'])
weighted_mode = repeat_values(df['MODE'], df['Count'])

###
df_plot = pd.DataFrame({
    'MODE': np.concatenate([weighted_mode] * 6),
    'Cost_Type': (['GHG_cost_190'] * len(weighted_GHG_cost_190) +
                  ['Air_pollutant_cost'] * len(weighted_Air_pollutant_cost) +
                  ['Collision_cost'] * len(weighted_Collision_cost))+
                  ['Congestion_cost'] * len(weighted_Congestion_cost)+
                  ['Noise_cost'] * len(weighted_Noise_cost)+
                  ['Externality'] * len(weighted_Externality),
    'Cost ($/trip)': np.concatenate([weighted_GHG_cost_190, weighted_Air_pollutant_cost, weighted_Collision_cost,
                                   weighted_Congestion_cost, weighted_Noise_cost, weighted_Noise_cost])
})

cost_type_labels = {
    'GHG_cost_190': 'GHG ($190/t-CO2)',
    'Air_pollutant_cost': 'Air pollutant',
    'Collision_cost': 'Collision',
    'Congestion_cost': 'Congestion',
    'Noise_cost': 'Noise',
    'Externality': 'Total social cost'
}
...
g = sns.catplot(x="MODE", y="Cost ($/trip)", hue='MODE',
                col="Cost_Type", data=df_plot, kind="box", height=4, aspect=.5,
